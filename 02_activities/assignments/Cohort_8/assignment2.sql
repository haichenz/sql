...
SELECT
product_name,
case when INSTR(product_name,'-') <> 0 THEN trim(substr(product_name, INSTR(product_name,'-') + 1)) else null end as description
FROM product;
...
select v.vendor_name, p.product_name, sum(sales)
FROM (
...
left join vendor v
on vvi.vendor_id = v.vendor_id
left join product p
on vvi.product_id = p.product_id
...
VALUES (24, 'hotpot base', '1 liter', 3, 'unit', CURRENT_TIMESTAMP);
...
SET current_quantity = vi_qty.qty
FROM
(SELECT pu.product_id, coalesce(vi.quantity, 0) as qty
...
on pu.product_id = vi.product_id) vi_qty
where product_units.product_id = vi_qty.product_id;
